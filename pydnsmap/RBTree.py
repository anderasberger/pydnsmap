...
# Updated by Andreas Berger <berger@ftw.at> (AB):
#       Added generator for tree iteration (traversalGenerator)
#       Removed <count> attribute to save memory
#       Use __slots__ for RBNode to save memory and make the object better
#       'pickleable'
#       Remove the dependency on the <string> module
...
class RBNode(object):
    ...
    __slots__=['left', 'right', 'parent', 'color', 'key', 'value', 'nonzero']
...
class RBTree(object):
    ...
    def insertNode(self, key, value):
        ...
        while current != self.sentinel:
            ...
            if rc == 0:
                # AB: the element is already in the tree, ignore that silently
                return current
...
# AB: we don't use the count attribute
...
#            return
    ...
    def traversalGenerator(self):
        """
        NOTE: do not try to change the tree while traversing it, this will
        fail
        """
        if self.root == self.sentinel:
            yield
